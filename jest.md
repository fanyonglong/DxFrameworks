...
## 模拟功能
模拟函数也称为“间谍”，因为它们可以让您监视由其他代码间接调用的函数的行为，而不仅仅是测试输出。您可以使用创建模拟功能jest.fn()。如果没有给出实现，则mock函数将undefined在调用时返回。
...
## expect期望
在编写测试时，通常需要检查值是否符合某些条件。expect通过一系列“匹配器”，您可以访问许多不同的东西。

对于由Jest社区维护的其他Jest匹配器，请查看jest-extended。
